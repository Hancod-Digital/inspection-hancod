...
const userName = userDetails?.name != "" ? userDetails?.name : userDetails?.email?.split('@')[0] ;
  ...
  const { reset, handleSubmit, formState: { isSubmitSuccessful, errors }, register } = methods;
  ...
  const onSubmitHandler: SubmitHandler<UserFormInput> = async (values) => {
    ...
    console.log(values);
      makeApiCall(
      async () => new StudentService().addStudent({
        ...values,
        avatar: fileBuffer ? "https://seqptsvnihezsfbnpkpz.supabase.co/storage/v1/object/public/"+await uploadImage() : "",
        added_by:userName
        
      }), {
      afterSuccess: () => {
        toastWithTimeout(ToastVariant.Success, "Operation successfull");
        setChanged(!changed)
        reset();
      },
      afterError: (err: any) => {
  ...
  return (
            ...
            <form className="space-y-4" noValidate autoComplete="off" onSubmit={handleSubmit(onSubmitHandler)}>
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="grid grid-cols-[200px_1fr] items-start gap-4">
                  <Label className='pt-3' htmlFor="name">Name</Label>
                  <div>
                ...
                <div className="grid grid-cols-[200px_1fr] items-start gap-4">
                  <Label className='pt-3' htmlFor="contact_number">Contact Number</Label>
                  <div>
                    <Input id="contact_number" {...register('contact_number')} />
                    {errors.contact_number && <p className="text-red-500 text-[13px] mt-1">{errors.contact_number.message}</p>}
                  </div>
                </div>
                
                <div className="grid grid-cols-[200px_1fr] items-start gap-4">
                  <Label className='pt-3' htmlFor="address">Address</Label>
                  <div>
                    <Input id="address" {...register('address')} />
                    {errors.address && <p className="text-red-500 text-[13px] mt-1">{errors.address.message}</p>}
                  </div>
                </div>

                {/* Gender Selection */}
                <div className="grid grid-cols-[200px_1fr] items-start gap-4">
                  <Label className='pt-3' htmlFor="gender">Gender</Label>
                  <div>
                  <Controller
      name="gender"
    ...
    />
                    ...
                    {errors.gender && <p className="text-red-500 text-[13px] mt-1">{errors.gender.message}</p>}
                  </div>
                </div>
              </div>
              <CardHeader>
          ...
          <div className="grid grid-cols-2 gap-6 ">
                ...
                <div className="grid grid-cols-[200px_1fr] items-start gap-4">
                  <Label className='pt-3' htmlFor="company">Company</Label>
                  <div>
                    <Input id="company" {...register('company')} />
                    {errors.company && <p className="text-red-500 text-[13px] mt-1">{errors.company.message}</p>}
                  </div>
                </div>
                
                <div className="grid grid-cols-[200px_1fr] items-start gap-4">
                  <Label className='pt-3' htmlFor="id_no">ID No</Label>
                  <div>
                    <Input id="id_no" {...register('id_no')} />
                    {errors.id_no && <p className="text-red-500 text-[13px] mt-1">{errors.id_no.message}</p>}
                  </div>
                </div>

                {/* Card No and Designation */}
                <div className="grid grid-cols-[200px_1fr] items-start gap-4">
                  <Label className='pt-3' htmlFor="card_no">Card No</Label>
                  <div>
                    <Input id="card_no" {...register('card_no')} />
                    {errors.card_no && <p className="text-red-500 text-[13px] mt-1">{errors.card_no.message}</p>}
                  </div>
                </div>

                <div className="grid grid-cols-[200px_1fr] items-start gap-4">
                  <Label className='pt-3' htmlFor="designation">Designation</Label>
                  <div>
                    <Input id="designation" {...register('designation')} />
                    {errors.designation && <p className="text-red-500 text-[13px] mt-1">{errors.designation.message}</p>}
                  </div>
                </div>

                {/* Model/Level and Issued On */}
                <div className="grid grid-cols-[200px_1fr] items-start gap-4">
                  <Label className='pt-3' htmlFor="model_level">Model/Level</Label>
                  <div>
                    <Input id="model_level" {...register('model_level')} />
                    {errors.model_level && <p className="text-red-500 text-[13px] mt-1">{errors.model_level.message}</p>}
                  </div>
                </div>

                <div className="grid grid-cols-[200px_1fr] items-start gap-4">
                  <Label className='pt-3' htmlFor="issued_on">Issued On</Label>
                  <div>
                    <Input id="issued_on" type="date" {...register('issued_on')} />
                    {errors.issued_on && <p className="text-red-500 text-[13px] mt-1">{errors.issued_on.message}</p>}
                  </div>
                </div>

                {/* Valid Until */}
                <div className="grid grid-cols-[200px_1fr] items-start gap-4">
                  <Label className='pt-3' htmlFor="valid_untill">Valid Until</Label>
                  <div>
                    <Input id="valid_untill" type="date" {...register('valid_untill')} />
                    {errors.valid_untill && <p className="text-red-500 text-[13px] mt-1">{errors.valid_untill.message}</p>}
                  </div>
                </div>
